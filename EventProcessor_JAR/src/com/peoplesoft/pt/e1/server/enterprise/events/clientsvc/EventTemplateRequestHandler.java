 ...
 */
class EventTemplateRequestHandler implements RequestHandler
{
    //=================================================
    // Non-public static class fields.
    //=================================================
    
    private static E1Logger sLogger = JdeLog.getE1Logger(EventTemplateRequestHandler.class.getName());
    ...
    {
        EventTemplateRequest templateRequest = (EventTemplateRequest)request;
        
        if (sLogger.isDebug())
        {
            sLogger.debug(LogUtils.SYS_EVENTPROCESSOR,
                "event template request, userId=" + templateRequest.getCredentials().getUsername(), null, null,null);
        }
        ...
        String template =
            service.getEventTemplate(
                templateRequest.getCredentials(),
                templateRequest.getHost(),
                templateRequest.getPort(),
                templateRequest.getCategory(),
                templateRequest.getType(),
                templateRequest.getEnvironment());

        //
        //  Build the response.
        //
        EventTemplateResponse response = new EventTemplateResponse();
        response.setTemplateXml(template);
        return response;
    }
}
